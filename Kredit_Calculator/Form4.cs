...
namespace Kredit_Calculator
{
    public partial class Form4 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            int count = 0;
            int index = Convert.ToInt32(trackBar1.Value);
            for (int i = 1; i <= Convert.ToInt32(trackBar1.Value); i++)
            {
                dataGridView1.Rows.Add();
                dataGridView1.Rows[count].Cells[0].Value = i;
                dataGridView1.Rows[count].Cells[1].Value = Math.Round(summa3[index], 3);
                count++;
                index--;
            }
        ...
        private void trackbar1_move_event(object sender, MouseEventArgs e)
        {
            label4.Text = Convert.ToString(trackBar1.Value)+" месяцев";
        }

        private void trackbar2_move_event(object sender, MouseEventArgs e)
        {
